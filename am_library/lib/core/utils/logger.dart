...
class AppLogger {
  ...
  static void initialize() {
    ...
    _isInitialized = true;
    am_ui.CommonLogger.info('AppLogger initialized for am_library',
        tag: 'Logger');
  }
  ...
  static void _updateLoggingConfig(Environment environment) {
    switch (environment) {
      case Environment.development:
        am_ui.CommonLogger.configure(
            enabled: true, minLevel: am_ui.LogLevel.info);
        break;
      ...
      case Environment.production:
        am_ui.CommonLogger.configure(
            enabled: false, minLevel: am_ui.LogLevel.error);
        break;
    }
  }
  ...
  static void warning(String message,
    ...
    am_ui.CommonLogger.warning(message,
        tag: tag, error: error, stackTrace: stackTrace);
  }

  static void error(String message,
      {String? tag, Object? error, StackTrace? stackTrace}) {
    am_ui.CommonLogger.error(message,
        tag: tag, error: error, stackTrace: stackTrace);
  }
  ...
  static void apiRequest(String method, String url,
      {String? tag, Map<String, dynamic>? headers, dynamic body}) {
    am_ui.CommonLogger.info('API Req: $method $url', tag: tag ?? 'API');
    if (headers != null)
      am_ui.CommonLogger.debug('Headers: $headers', tag: tag ?? 'API');
    if (body != null)
      am_ui.CommonLogger.debug('Body: $body', tag: tag ?? 'API');
  }

  static void apiResponse(String method, String url, int statusCode,
      {String? tag, dynamic body, int? duration}) {
  ...
  static void userAction(String action,
    ...
    am_ui.CommonLogger.info('User Action: $action context: $context',
        tag: tag ?? 'UserAction');
  }

  static void stateChange(String from, String to,
      {String? tag, dynamic event}) {
    am_ui.CommonLogger.debug('State: $from -> $to event: $event',
        tag: tag ?? 'State');
  }
}
