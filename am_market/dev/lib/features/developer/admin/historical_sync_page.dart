...
class _HistoricalSyncPageState extends State<HistoricalSyncPage>
  ...
  bool _fetchIndexStocks =
      false; // Whether to fetch individual stocks from index symbols
    ...
    if (symbol.isEmpty) {
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text("Please select an Index or enter a Symbol")));
      return;
    }

    CommonLogger.info(
        "Triggering sync for $symbol (Force: $_forceRefresh, Fetch Index Stocks: $_fetchIndexStocks)",
        tag: "Admin");
  ...
  Future<void> _stopIngestion() async {
    try {
      await _adminService
          .stopIngestion(_selectedProvider); // Uses selected provider
      _pausePolling(); // Stop polling when stopping ingestion
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text("Ingestion Stopped Successfully! (Polling paused)")));
    } catch (e) {
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text("Failed to stop ingestion: $e"),
            backgroundColor: Colors.red));
    }
  }
  ...
  void _startStream() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Feed Started (Simulated)")));
  }

  void _stopStream() {
    setState(() => _isStreaming = false);
    ScaffoldMessenger.of(context)
        .showSnackBar(const SnackBar(content: Text("Feed Stopped")));
  }
            ...
            child: Column(
              ...
              children: [
                ...
                Wrap(
                  ...
                  runSpacing: 10,
                  children: [
                    _detailItem("Status", job.status,
                        color: job.status == 'SUCCESS'
                            ? Colors.green
                            : Colors.blue),
                    _detailItem("Duration",
                        "${(job.durationMs / 1000).toStringAsFixed(1)}s"),
                    _detailItem("Processed", "${job.totalSymbols}"),
                    _detailItem("Success", "${job.successCount}",
                        color: Colors.green),
                    _detailItem("Failed", "${job.failureCount}",
                        color: Colors.red),
                ...
                if (job.failedSymbols.isNotEmpty) ...[
                  ...
                  Container(
                    ...
                    child: Text(job.failedSymbols.join(", "),
          ...
          if (_currentJob != null) ...[
            Wrap(
              ...
              children: [
                _buildGlossyCard(
                    "Symbols Processed",
                    "${_currentJob!.totalSymbols}",
                    Icons.bar_chart,
                    Colors.blue,
                    theme),
                _buildGlossyCard(
                    "Payload Size",
                    _formatBytes(_currentJob!.payloadSize),
                    Icons.data_usage,
                    Colors.purple,
                    theme),
                _buildGlossyCard("Success", "${_currentJob!.successCount}",
                    Icons.check_circle, Colors.green, theme),
                _buildGlossyCard("Failed", "${_currentJob!.failureCount}",
                    Icons.error, Colors.red, theme),
              ],
            ),
              ...
              Row(
                children: [
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: _isPollingPaused
                          ? Colors.orange.withOpacity(0.1)
                          : Colors.green.withOpacity(0.1),
                      borderRadius: BorderRadius.circular(20),
                      border: Border.all(
                          color:
                              _isPollingPaused ? Colors.orange : Colors.green),
                    ),
                    child: Row(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                      ...
                      child: DataTable(
                        ...
                        rows: _logs
                            .map((log) => DataRow(cells: [
                                  DataCell(Text(log.jobId.length > 8
                                      ? log.jobId.substring(0, 8)
                                      : log.jobId)),
                                  DataCell(Text(DateFormat('MMM dd, HH:mm:ss')
                                      .format(log.startTime))),
                                  DataCell(_buildStatusBadge(log.status)),
                                  DataCell(Text(
                                      "${(log.durationMs / 1000).toStringAsFixed(1)}s")),
                                  DataCell(Text("${log.successCount}",
                                      style: const TextStyle(
                                          color: Colors.green,
                                          fontWeight: FontWeight.bold))),
                                  DataCell(Text("${log.failureCount}",
                                      style: const TextStyle(
                                          color: Colors.red,
                                          fontWeight: FontWeight.bold))),
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        child: DropdownButton<String>(
                            ...
                            style: theme.textTheme.bodyMedium,
                            dropdownColor: theme.cardColor,
                            items: ["NIFTY 50", "NIFTY BANK", "NIFTY 500"]
                                .map((e) =>
                                    DropdownMenuItem(value: e, child: Text(e)))
                                .toList(),
                            onChanged: (val) {
                        ...
                        child: Row(
                          ...
                          children: [
                            Checkbox(
                              value: _forceRefresh,
                              onChanged: (val) =>
                                  setState(() => _forceRefresh = val ?? true),
                            ),
                            Text("Force Refresh",
                                style: theme.textTheme.bodyMedium),
                          ],
                      ...
                      InkWell(
                        onTap: () => setState(
                            () => _fetchIndexStocks = !_fetchIndexStocks),
                        child: Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            Checkbox(
