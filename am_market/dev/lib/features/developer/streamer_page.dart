...
class _StreamerPageState extends ConsumerState<StreamerPage> {
  ...
  void didChangeDependencies() {
    ...
    try {
      _marketProvider =
          legacy_provider.Provider.of<MarketProvider>(context, listen: false);
    } catch (e) {
      _log("didChangeDependencies Error: $e",
          method: "StreamerPage.didChangeDependencies", level: LogLevel.error);
    }
  }
  ...
  Future<void> _loadDefaultIndices() async {
    try {
      ...
      if (allSymbols.isNotEmpty) {
        _log(
            "Fetching details from batch API: https://am.asrax.in/market/v1/indices/batch?forceRefresh=false");
        final batchData = await _apiService.fetchIndicesBatch(allSymbols,
            forceRefresh: false);
        ...
        if (uniqueSymbols.isNotEmpty && mounted) {
          ...
          _log(
              "Loaded ${uniqueSymbols.length} symbols (indices + constituents) from batch API as default.");
          return;
        }
      }
      ...
      if (mounted) {
        setState(() {
          _symbolsController.text =
              'NIFTY 50, NIFTY BANK, NIFTY NEXT 50, NIFTY 100, INDIA VIX';
        });
      }
    }
  }
  ...
  void _handleUpdate(MarketDataUpdate update) {
    ...
    try {
      ...
      _log("Error processing update: $e",
          method: "StreamerPage._handleUpdate", level: LogLevel.error);
    }
  }
  ...
  void _log(String msg,
      {String method = 'StreamerPage', LogLevel level = LogLevel.info}) {
    if (mounted) {
      setState(() {
        _logs.insert(
            0, "[${DateFormat('HH:mm:ss').format(DateTime.now())}] $msg");
      ...
      try {
        ...
        if (await canLaunchUrl(uri)) {
          ...
          _log("Could not launch URL",
              method: "StreamerPage._getLoginUrl", level: LogLevel.error);
          showDialog(
            context: context,
  ...
  Future<void> _startStream() async {
    ...
    if (raw.isEmpty) {
      _log("Please enter symbols",
          method: "StreamerPage._startStream", level: LogLevel.warning);
      return;
    }

    List<String> symbols = raw
        .split(',')
    ...
    if (priceService == null) {
      _log("PriceService not ready",
          method: "StreamerPage._startStream", level: LogLevel.error);
      return;
    }
    ...
    try {
      ...
      _log(
          "Stream Subscription Sent via PriceService: OK (isIndexSymbol: $_isIndexSymbol, mockMode: $_mockMode)",
          method: "StreamerPage._startStream");
      ...
      _log("Stream Subscription Failed: $e",
          method: "StreamerPage._startStream", level: LogLevel.error);
    }
  }
  ...
  Future<void> _stopStream() async {
    ...
    setState(() => _isStreaming = false);
    _log("Stopped Monitoring (Explicit disconnect disabled)",
        method: "StreamerPage._stopStream");
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text("Market Data Streamer",
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
                  ...
                  items: [
                    ...
                    'None'
                      ...
                      .map((e) => DropdownMenuItem(
                          value: e,
                          child:
                              Text(e == 'NSE_EQ' ? 'NSE Equity (NSE_EQ)' : e)))
                      .toList(),
            ...
            Text("Enter symbols without exchange if Auto-prefix is on.",
              ...
              children: [
                ...
                if (_searchResults.isEmpty &&
                  ...
                  Padding(
                    ...
                    child: Center(
                      child: Text(
                          "Enter a query to find instruments or start a stream.",
          ...
          return ListTile(
            title: Text(symbol,
                style: theme.textTheme.bodyLarge
                    ?.copyWith(fontWeight: FontWeight.bold)),
            subtitle:
                Text("$name ($exchange)", style: theme.textTheme.bodySmall),
            trailing: IconButton(
  ...
  Widget _buildLiveFeedSection(BuildContext context) {
    ...
    final endIndex = calculateEndIndex < _feedHistory.length
        ? calculateEndIndex
        : _feedHistory.length;
              ...
              child: SingleChildScrollView(
                ...
                child: Theme(
                  ...
                  child: SizedBox(
                    ...
                    child: DataTable(
                      headingRowColor:
                          MaterialStateProperty.all(theme.canvasColor),
                      ...
                      rows: currentItems.map((data) {
                        ...
                        return DataRow(
                          cells: [
                            ...
                            DataCell(Text(change.toStringAsFixed(2),
                                style: TextStyle(
                                    color: color,
                                    fontWeight: FontWeight.bold))),
                            DataCell(Text('${pChange.toStringAsFixed(2)}%',
                                style: TextStyle(
                                    color: color,
                                    fontWeight: FontWeight.bold))),
                            DataCell(Text(prevClose.toStringAsFixed(2),
