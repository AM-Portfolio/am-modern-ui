...
class EtfDetailPage extends StatefulWidget {
  ...
  const EtfDetailPage(
      {super.key, required this.symbol, required this.name, this.onBack});
...
class _EtfDetailPageState extends State<EtfDetailPage> {
        ...
        if (triggered) {
          ...
          if (mounted) {
            setState(() {
              ...
              _isLoading = false;
              if (retry == null)
                _error = "Fetching data... please check back later.";
            });
          }
        } else {
          if (mounted)
  ...
  Widget _buildContent() {
    ...
    // Calculate totals
    double totalPercent =
        _holdings!.holdings.fold(0, (sum, item) => sum + item.percentage);
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
              "Holdings Breakdown (${_holdings!.holdings.length}) - Total: ${totalPercent.toStringAsFixed(2)}%",
              style:
            ...
            child: DataTable(
              ...
              rows: _holdings!.holdings.map((h) {
                return DataRow(cells: [
                  ...
                  DataCell(Text(h.marketValue != null
            ...
            Column(
              ...
              children: [
                Text("Holdings Count",
                    style:
                        TextStyle(color: Colors.grey.shade600, fontSize: 12)),
                Text("${_holdings?.holdingsCount ?? 0}",
                    style: const TextStyle(
                        fontWeight: FontWeight.bold, fontSize: 16)),
              ],
            ),
