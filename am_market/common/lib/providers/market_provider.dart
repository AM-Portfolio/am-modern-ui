...
class MarketProvider with ChangeNotifier {
            ...
            if (val is Map && val.containsKey('dataPoints')) {
              ...
              if (points.isNotEmpty) {
                for (int i = 0; i < points.length; i++) {
                  ...
                  final p =
                      point['close'] ?? point['lastPrice'] ?? point['price'];
  ...
  Future<void> loadIndices() async {
    ...
    try {
      ...
      CommonLogger.info(
          "Fetched available indices: ${_availableIndices?.broad.length ?? 0} broad, ${_availableIndices?.sectoral.length ?? 0} sectoral",
  ...
  Future<void> loadAllIndicesData() async {
    ...
    try {
      ...
      CommonLogger.info("Loading ${allSymbols.length} indices: $allSymbols",
          tag: "MarketProvider.loadAllIndicesData");

      // STEP 3: Call batch endpoint with the available symbols
      _allIndicesData = await _apiService.fetchIndicesBatch(allSymbols,
          forceRefresh: _forceRefresh);

      CommonLogger.info("Successfully loaded ${_allIndicesData.length} indices",
          tag: "MarketProvider.loadAllIndicesData");

      await _ensurePriceSubscription(allSymbols);
      _syncWithPriceService();
    } catch (e) {
      CommonLogger.error("Error loading all indices data",
  ...
  Future<void> loadHeatmap(String symbol, String timeframe) async {
    ...
    try {
      _heatmapValues =
          await _apiService.fetchHeatmap(symbol, timeframe: timeframe);
      notifyListeners();
    } catch (e) {
