...
class CommonPerformanceChart extends StatefulWidget {
  ...
  final double
      minPointWidth; // Pixel spacing per point when scrolling is enabled
...
class _CommonPerformanceChartState extends State<CommonPerformanceChart> {
  ...
  Widget build(BuildContext context) {
    ...
    final int dataLength =
        hasMultiLines ? widget.lines!.first.points.length : activeData.length;
          ...
          children: [
            Padding(
              padding: EdgeInsets.only(
                  top: showToggle ? 40.0 : 16.0,
                  right: 0.0), // Added spacing here!
              child: LayoutBuilder(
                builder: (context, constraints) {
                  final double calculatedWidth =
                      dataLength * widget.minPointWidth;
                  final bool needsScroll = widget.enableScrolling &&
                      (calculatedWidth > constraints.maxWidth);

                  final double chartWidth =
                      needsScroll ? calculatedWidth : constraints.maxWidth;
          ...
          Wrap(
            ...
            children: widget.lines!.map((line) {
              final Color bulletColor =
                  line.color ?? widget.chartColor ?? AppColors.primary;
              return Row(
