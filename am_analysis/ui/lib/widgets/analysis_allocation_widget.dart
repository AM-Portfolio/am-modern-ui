...
class _AnalysisAllocationWidgetState extends State<AnalysisAllocationWidget> {
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final height = widget.height ?? (isMobile ? 500 : isTablet ? 480 : 450);
  ...
  Widget _buildContent(bool isMobile, bool isTablet) {
    ...
    final contentItems = [
      ...topItems.asMap().entries.map((entry) {
        ...
        return _buildAllocationItem(index, item, isMobile);
      }),
      if (_items.length > 5)
        Padding(
  ...
  Widget _buildAllocationItem(int index, AllocationItem item, bool isMobile) {
  ...
  Widget _buildStackedBar(bool isMobile) {
    ...
    return Container(
      ...
      child: ClipRRect(
        borderRadius: BorderRadius.circular(isMobile ? 8 : 10),
        child: Row(
          children: _items.where((i) => i.percentage > 0).map((item) {
            ...
            // Ensure flex is at least 1 for any item > 0
            final flexValue = (item.percentage * 100).round().clamp(1, 100);
