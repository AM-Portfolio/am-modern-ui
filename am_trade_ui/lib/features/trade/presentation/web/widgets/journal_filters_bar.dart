...
class JournalFiltersBar extends StatelessWidget {
            ...
            children: [
              ...
              SegmentedButton<String>(
                ...
                onSelectionChanged: (newSelection) =>
                    onFilterLogicChanged(newSelection.first),
                style: ButtonStyle(
                  textStyle:
            ...
            children: [
              TextButton.icon(
                ...
                icon: Icon(
                    ...
                    size: 14),
                label: Text(showAdvancedFilters ? 'Less' : 'More',
        ...
        children: [
          Expanded(
            ...
            child: _buildDropdownFilter(
              ...
              onChanged: (value) =>
                  onYearChanged(value != null ? int.parse(value) : null),
            ),
          ),
          const SizedBox(width: 8),
          Expanded(
            ...
            child: _buildDropdownFilter(
              ...
              onChanged: (value) =>
                  onMonthChanged(value != null ? int.parse(value) : null),
            ),
          ),
          const SizedBox(width: 8),
          Expanded(
            ...
            child: FilterChip(
              selected: showLast20,
              label:
                  const Text('Last 20 Entries', style: TextStyle(fontSize: 11)),
  ...
  Widget _buildMoodAndSentimentRow(ThemeData theme) => Row(
        ...
        children: [
          Expanded(
              child:
                  _buildFilterCategory(theme, 'Mood', _buildMoodChips(theme))),
          const SizedBox(width: 12),
          Expanded(
              child: _buildFilterCategory(
                  theme, 'Market Sentiment', _buildSentimentChips(theme))),
        ],
      );

  Widget _buildFilterCategory(
          ThemeData theme, String title, List<Widget> chips) =>
      Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(
  ...
  List<Widget> _buildMoodChips(ThemeData theme) =>
      JournalMoodOptions.moods.entries.map((entry) {
        ...
        return FilterChip(
          ...
          side: BorderSide(
            color:
                isSelected ? entry.value['color'] as Color : theme.dividerColor,
            width: isSelected ? 1.5 : 1,
          ),
        );
      }).toList();

  List<Widget> _buildSentimentChips(ThemeData theme) =>
      JournalMoodOptions.sentiments.entries.map((entry) {
        ...
        return FilterChip(
          ...
          label: Text(entry.value['label'] as String,
              style: const TextStyle(fontSize: 11)),
          onSelected: (selected) =>
              onSentimentChanged(selected ? entry.key : null),
          padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
          backgroundColor: isSelected
              ? (entry.value['color'] as Color).withOpacity(0.15)
              : null,
  ...
  List<Widget> _buildTagChips(ThemeData theme) =>
      JournalMoodOptions.tags.map((tagData) {
        ...
        return FilterChip(
          ...
          padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
          backgroundColor:
              isSelected ? (tagData['color'] as Color).withOpacity(0.15) : null,
          selectedColor: (tagData['color'] as Color).withOpacity(0.2),
          checkmarkColor: tagData['color'] as Color,
          side: BorderSide(
              color:
                  isSelected ? tagData['color'] as Color : theme.dividerColor,
              width: isSelected ? 1.5 : 1),
        );
      }).toList();
