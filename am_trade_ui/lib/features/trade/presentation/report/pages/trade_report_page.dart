...
// ... (imports)
  ...
  void _applyFilter(MetricsFilterConfig config) async {
    ...
    final request = MetricsFilterRequest(
      ...
      startDate:
          config.dateRange?.startDate ?? DateTime(DateTime.now().year, 1, 1),
  ...
  ChartTimeFrame _getAutoTimeFrame() {
    final start = _currentConfig.dateRange?.startDate ??
  ...
  Future<void> _showDatePicker(BuildContext context) async {
    final picked = await showDialog<DateTimeRange>(
      ...
      builder: (context) => CompactDateRangePickerDialog(
        initialDateRange: _currentConfig.dateRange != null
            ? DateTimeRange(
                start: _currentConfig.dateRange!.startDate,
                end: _currentConfig.dateRange!.endDate)
            : null,
      ),
    );
    ...
    if (picked != null) {
      _applyFilter(MetricsFilterConfig(
          dateRange:
              DateRangeFilter(startDate: picked.start, endDate: picked.end)));
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: cubitAsync.when(
        ...
        error: (error, stack) =>
            Center(child: Text('Error initializing report: $error')),
      ),
    );
  }
      ...
      children: [
        ...
        LayoutBuilder(builder: (context, constraints) {
          ...
          final double itemWidth =
              (constraints.maxWidth - 36) / 4; // 3 gaps of 12px
          ...
          return Wrap(
            ...
            children: [
              ...
              _buildCompactMetric(
                  ...
                  state.summary.totalProfitLoss >= 0
                      ? Icons.trending_up
                      : Icons.trending_down,
                  state.summary.totalProfitLoss >= 0
                      ? Colors.green
              ...
              _buildCompactMetric(
                  'Profit Factor',
                  (state.summary.profitFactor.isInfinite ||
                          state.summary.profitFactor.isNaN)
                      ? '∞'
                      : state.summary.profitFactor.toStringAsFixed(2),
                  Icons.scale,
                  Colors.orange,
                  width: itemWidth),
              _buildCompactMetric(
              ...
              _buildCompactMetric(
                  'Avg Win/Loss',
                  state.summary.metrics.avgTradeWinLossRatio
                          ?.toStringAsFixed(2) ??
                      '0.00',
              ...
              _buildCompactMetric(
                  'Expectancy',
                  (state.summary.metrics.avgGrossTradePnL ?? 0)
                      .toStringAsFixed(2),
                  Icons.analytics,
