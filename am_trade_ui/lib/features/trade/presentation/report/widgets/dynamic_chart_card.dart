...
class _DynamicChartCardState extends ConsumerState<DynamicChartCard> {
  ...
  List<ChartDataPoint> _getDataForMetric(ChartMetric metric) {
    ...
    switch (_selectedTimeFrame) {
      ...
      case ChartTimeFrame.dailyLinear:
        points =
            ChartAggregator.accumulateDaily(widget.dailyPerformance, metric);
        break;
      case ChartTimeFrame.weeklyLinear:
        points =
            ChartAggregator.accumulateWeekly(widget.dailyPerformance, metric);
        break;
      case ChartTimeFrame.monthlyLinear:
        points =
            ChartAggregator.accumulateMonthly(widget.dailyPerformance, metric);
        break;
      ...
      case ChartTimeFrame.hourSeason:
        ...
        for (var hour in sortedKeys) {
          ...
          final aggValue = _aggregateSeasonality(
              items.map((e) => e.metrics).toList(),
              items.map((e) => e.tradeCount).toList(),
              metric);
          String label = '$hour';
          points.add(
              ChartDataPoint(xLabel: label, yValue: aggValue, xIndex: hour));
        }
        break;
      ...
      case ChartTimeFrame.daySeason:
        ...
        for (var dayOrder in sortedKeys) {
          ...
          points.add(ChartDataPoint(
              xLabel: label, yValue: aggValue, xIndex: dayOrder));
        }
        break;
      ...
      case ChartTimeFrame.monthSeason:
        ...
        for (var monthOrder in sortedKeys) {
          ...
          points.add(ChartDataPoint(
              xLabel: label, yValue: aggValue, xIndex: monthOrder));
        }
        break;
      ...
      case ChartTimeFrame.yearSeason:
        ...
        for (var i = 0; i < data.length; i++) {
          final item = data[i];
          final val =
              metric.getValue(item.metrics, tradeCount: item.tradeCount);
          points.add(
              ChartDataPoint(xLabel: '${item.year}', yValue: val, xIndex: i));
        }
        break;
    }
  ...
  Widget build(BuildContext context) {
    ...
    final firstMetricData =
        allData.values.isNotEmpty ? allData.values.first : <ChartDataPoint>[];
                  ...
                  CompositedTransformTarget(
                    ...
                    child: InkWell(
                      ...
                      child: Container(
                        ...
                        child: Row(children: [
                          Text('${_selectedMetrics.length} Metrics',
                              style: TextStyle(
  ...
  double _aggregateSeasonality(List<ReportPerformanceMetrics> metricsList,
      List<int> tradeCounts, ChartMetric metric) {
  ...
  void _showDropdown() {
    ...
    _dropdownOverlay = OverlayEntry(builder: (context) {
                                                            ...
                                                            ?.copyWith(
                                                                fontWeight:
                                                                    FontWeight
                                                                        .bold)),
                                                    Text(
                                                        "${_selectedMetrics.length} Active",
                                                        style: TextStyle(
                                                            fontSize: 10,
                                                            color: Theme.of(
                                                                    context)
                                                                ...
                                                                child: Row(
                                                                    children: [
                                                                      AnimatedContainer(
                                                                        ...
                                                                        decoration: BoxDecoration(
                                                                            color: isSelected
                                                                                ? _metricColors[metric]
                                                                                : Colors.transparent,
                                                                            borderRadius: BorderRadius.circular(5),
                                                                            border: Border.all(color: isSelected ? Colors.transparent : Theme.of(context).colorScheme.onSurface.withOpacity(0.4), width: 1.5)),
                                                                        child: isSelected
  ...
  Widget _buildChart(Map<ChartMetric, List<ChartDataPoint>> allData,
      List<ChartDataPoint> firstMetricData, ThemeData theme) {
          ...
          getTooltipItem: (group, groupIndex, rod, rodIndex) {
            ...
            if (metric.label.toLowerCase().contains('pnl') ||
              ...
              if (!metric.label.toLowerCase().contains('rate') &&
                  !metric.label.toLowerCase().contains('count')) {
                formattedValue =
                    '${currency.symbol}${rod.toY.toStringAsFixed(2)}';
              }
            }
            if (metric.label.toLowerCase().contains('rate') ||
                metric.label.toLowerCase().contains('percentage')) {
    ...
    return LineChart(LineChartData(
      ...
      lineBarsData: _selectedMetrics.map((metric) {
        ...
        return LineChartBarData(
          spots:
              points.map((p) => FlSpot(p.xIndex.toDouble(), p.yValue)).toList(),
          isCurved: true,
          ...
          return touchedSpots.map((spot) {
            ...
            if (metric.label.toLowerCase().contains('pnl') ||
              ...
              if (!metric.label.toLowerCase().contains('rate') &&
                  !metric.label.toLowerCase().contains('count')) {
                formattedValue =
                    '${currency.symbol}${spot.y.toStringAsFixed(2)}';
              }
            }
            if (metric.label.toLowerCase().contains('rate') ||
                metric.label.toLowerCase().contains('percentage')) {
          ...
          getTitlesWidget: (val, meta) {
            ...
            if (index >= 0 && index < firstMetricData.length) {
              return Padding(
                ...
                child: Text(firstMetricData[index].xLabel,
                    style: const TextStyle(
  ...
  List<BarChartGroupData> _buildBarGroups(
      Map<ChartMetric, List<ChartDataPoint>> allData) {
    ...
    for (var metric in _selectedMetrics) {
      ...
      for (var p in points) {
        ...
        groups[p.xIndex]!.add(BarChartRodData(
            toY: p.yValue,
