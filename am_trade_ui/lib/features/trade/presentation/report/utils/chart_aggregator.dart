...
class ChartAggregator {
  static List<ChartDataPoint> accumulateDaily(
      List<DailyPerformance> dailyData, ChartMetric metric) {
    // Sort by date just in case
    final sortedData = List.of(dailyData)
  ...
  static List<ChartDataPoint> accumulateWeekly(
      List<DailyPerformance> dailyData, ChartMetric metric) {
    ...
    return List.generate(sortedKeys.length, (index) {
      ...
      final label =
          'W${DateFormat('w').format(labelDate)}\n${DateFormat('yy').format(labelDate)}';

      return ChartDataPoint(xLabel: label, yValue: aggValue, xIndex: index);
    });
  }

  static List<ChartDataPoint> accumulateMonthly(
      List<DailyPerformance> dailyData, ChartMetric metric) {
    if (dailyData.isEmpty) return [];
    ...
    while (current.isBefore(maxDate) ||
        current.isAtSameMomentAs(maxDate) ||
      ...
      result.add(
          ChartDataPoint(xLabel: label, yValue: aggValue, xIndex: index++));
  ...
  static double _aggregateMetric(
      List<DailyPerformance> items, ChartMetric metric) {
