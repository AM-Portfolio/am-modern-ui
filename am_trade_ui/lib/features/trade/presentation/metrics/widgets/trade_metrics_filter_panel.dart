...
class TradeMetricsFilterPanel extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<TradeMetricsFilterPanel> createState() =>
      _TradeMetricsFilterPanelState();
}

class _TradeMetricsFilterPanelState
    extends ConsumerState<TradeMetricsFilterPanel>
  ...
  void _applyFilters() {
    final config = MetricsFilterConfig(
      ...
      profitLossFilters: _activeGroups
          ...
          .firstOrNull
          ?.toFilterCriteria(),
      metricTypes: _activeGroups
              .whereType<MetricTypeFilterGroup>()
              .firstOrNull
              ?.selectedTypes ??
          [],
    );
                          ...
                          child: Row(
                            ...
                            children: [
                              PopupMenuButton<MetricsFilterGroupType>(
                                itemBuilder: (context) => [
                                  if (!_activeGroups
                                      .any((g) => g is DateRangeFilterGroup))
                                    PopupMenuItem(
                                  ...
                                  if (!_activeGroups
                                      .any((g) => g is InstrumentFilterGroup))
                                    PopupMenuItem(
                                  ...
                                  if (!_activeGroups.any((g) =>
                                      g is TradeCharacteristicsFilterGroup))
                                    PopupMenuItem(
                                      value: MetricsFilterGroupType
                                          .tradeCharacteristics,
                                      child: _buildMenuTile(
                                  ...
                                  if (!_activeGroups
                                      .any((g) => g is ProfitLossFilterGroup))
                                    PopupMenuItem(
                                  ...
                                  if (!_activeGroups
                                      .any((g) => g is MetricTypeFilterGroup))
                                    PopupMenuItem(
      ...
      builder: (context, constraints) {
        ...
        return isMobile
            ? Wrap(
                spacing: 8,
                runSpacing: 8,
                children: _activeGroups
                    .asMap()
                    .entries
                    .map((e) => SizedBox(
                        width: constraints.maxWidth,
                        child: _buildFilterGroupCard(e.key, e.value)))
                    .toList())
            : Row(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: _activeGroups
