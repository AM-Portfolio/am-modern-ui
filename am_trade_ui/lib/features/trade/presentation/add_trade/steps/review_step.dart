...
class ReviewStep extends StatelessWidget {
        ...
        children: [
          ...
          _buildCompactReviewCard(theme, 'Trade Summary', Icons.summarize, [
            ...
            _buildReviewRow(
              ...
              selectedDirection == TradeDirections.long
                  ? Icons.trending_up
                  : Icons.trending_down,
              color: selectedDirection == TradeDirections.long
                  ? Colors.green
                  : Colors.red,
            ),
            _buildReviewRow(
              ...
              Icons.flag,
              color: selectedStatus == TradeStatuses.open
                  ? Colors.orange
                  : Colors.green,
            ),
          ]),
          ...
          _buildCompactReviewCard(
              theme, 'Transaction Details', Icons.receipt_long, [
            // Entry
            Row(
              children: [
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      if (entryDate != null)
                        _buildCompactRow(theme, 'Date',
                            entryDate!.toLocal().toString().split(' ')[0]),
                      _buildCompactRow(theme, 'Price', '₹$entryPrice'),
                      _buildCompactRow(theme, 'Qty', entryQuantity),
                    ],
                  ),
                ),
                if (selectedStatus != TradeStatuses.open &&
                    exitDate != null) ...[
                  Container(
                      width: 1,
                      height: 60,
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        if (exitDate != null)
                          _buildCompactRow(theme, 'Date',
                              exitDate!.toLocal().toString().split(' ')[0]),
                        if (exitPrice.isNotEmpty)
                          _buildCompactRow(theme, 'Price', '₹$exitPrice'),
                        if (exitQuantity.isNotEmpty)
                          _buildCompactRow(theme, 'Qty', exitQuantity),
                      ],
                    ),
                  ),
                ],
            ...
            if (strategy.isNotEmpty)
              _buildReviewRow(theme, 'Strategy', strategy, Icons.psychology),
          ]),
          ...
          if (selectedDerivativeType != null) ...[
            ...
            _buildCompactReviewCard(theme, 'Derivative', Icons.analytics, [
              ...
              if (strikePrice.isNotEmpty)
                _buildReviewRow(theme, 'Strike', strikePrice, Icons.gavel),
          ...
          if (attachments.isNotEmpty) ...[
            ...
            _buildCompactReviewCard(theme, 'Attachments', Icons.attach_file, [
              AttachmentPreviewGrid(
                  attachments:
                      attachments.map(AttachmentItem.uploaded).toList(),
                  readOnly: true),
            ]),
          ],
  ...
  Widget _buildCompactReviewCard(ThemeData theme, String title, IconData icon,
  ...
  Widget _buildCompactRow(ThemeData theme, String label, String value) =>
      Padding(
  ...
  Widget _buildChipRow(ThemeData theme, String label, List<String> items) =>
      Padding(
