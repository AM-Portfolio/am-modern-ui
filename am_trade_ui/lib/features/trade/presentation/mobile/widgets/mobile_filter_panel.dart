...
class _FilterBottomSheetContent extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_FilterBottomSheetContent> createState() =>
      _FilterBottomSheetContentState();
}

class _FilterBottomSheetContentState
    extends ConsumerState<_FilterBottomSheetContent>
  ...
  void _initializeInstrumentFilter(MetricsFilterConfig config) {
    ...
    _instrumentGroup!.selectedSegments =
        List.from(config.instrumentFilters!.marketSegments);
    _instrumentGroup!.selectedIndexTypes =
        List.from(config.instrumentFilters!.indexTypes);
    _instrumentGroup!.selectedDerivativeTypes =
        List.from(config.instrumentFilters!.derivativeTypes);
    _instrumentGroup!.symbolsController.text =
  ...
  void _initializeTradeCharFilter(MetricsFilterConfig config) {
    ...
    _tradeCharGroup!.strategiesController.text =
        tradeChar.strategies.join(', ');
    _tradeCharGroup!.tagsController.text = tradeChar.tags.join(', ');

    if (tradeChar.minHoldingTimeHours != null) {
      _tradeCharGroup!.minHoldingHoursController.text =
          tradeChar.minHoldingTimeHours.toString();
    }
    if (tradeChar.maxHoldingTimeHours != null) {
      _tradeCharGroup!.maxHoldingHoursController.text =
          tradeChar.maxHoldingTimeHours.toString();
    }
  }
  ...
  void _initializeProfitLossFilter(MetricsFilterConfig config) {
    ...
    if (pnl.minPositionSize != null) {
      _profitLossGroup!.minPositionSizeController.text =
          pnl.minPositionSize.toString();
    }
    if (pnl.maxPositionSize != null) {
      _profitLossGroup!.maxPositionSizeController.text =
          pnl.maxPositionSize.toString();
    }
  }
    ...
    showDialog(
      ...
      builder: (dialogContext) => BlocProvider.value(
        ...
        child: AlertDialog(
          ...
          content:
              _buildSaveDialogContent(nameController, descriptionController),
          actions: _buildSaveDialogActions(
              dialogContext, nameController, descriptionController, cubit),
        ),
      ),
    );
  }

  Widget _buildSaveDialogContent(TextEditingController nameController,
  ...
  void _handleSaveFilter(
    ...
    ScaffoldMessenger.of(context)
        .showSnackBar(SnackBar(content: Text('Saved filter "$name"')));
  }
        ...
        builder: (context, state) => state.when(
          ...
          loaded: (filterList, selectedFilter) {
            ...
            return _buildFavoriteButton(
                Theme.of(context), filterList, selectedFilter);
          },
  ...
  Widget _buildFavoriteButton(ThemeData theme, FavoriteFilterList filterList,
      ...
      PopupMenuButton<String>(
        ...
        itemBuilder: (context) =>
            _buildFavoriteMenuItems(theme, filterList, selectedFilter),
        onSelected: (value) => _handleFavoriteSelection(value, filterList),
      );

  Widget _buildFavoriteIcon(ThemeData theme, FavoriteFilter? selectedFilter) =>
      Container(
        padding: const EdgeInsets.all(8),
        decoration: BoxDecoration(
        ...
        child: Icon(Icons.bookmark_rounded,
            size: 22,
            color:
                selectedFilter != null ? theme.primaryColor : theme.hintColor),
      );
  ...
  PopupMenuItem<String> _buildFavoriteMenuHeader(
          ThemeData theme, FavoriteFilterList filterList) =>
      PopupMenuItem<String>(
        ...
        child: Row(
          children: [
            ...
            const Spacer(),
            Text('${filterList.filters.length}',
  ...
  Widget _buildFilterListItemContent(
          ThemeData theme, FavoriteFilter filter, bool isSelected) =>
      Row(
        children: [
  ...
  Widget _buildSelectionIcon(ThemeData theme, bool isSelected) => isSelected
      ? Icon(Icons.check_circle, size: 18, color: theme.primaryColor)
      : const SizedBox(width: 18);

  Widget _buildFilterOptionsMenu(ThemeData theme, FavoriteFilter filter) =>
      PopupMenuButton<String>(
        icon: Icon(Icons.more_vert, size: 18, color: theme.hintColor),
        tooltip: 'Filter options',
        itemBuilder: (context) => [
          if (!filter.isDefault) _buildSetDefaultMenuItem(theme),
          _buildDeleteMenuItem(theme)
        ],
        onSelected: (action) => _handleFilterAction(action, filter),
      );

  PopupMenuItem<String> _buildSetDefaultMenuItem(ThemeData theme) =>
      PopupMenuItem<String>(
        value: 'set_default',
        padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
        child: Row(
  ...
  PopupMenuItem<String> _buildDeleteMenuItem(ThemeData theme) =>
      PopupMenuItem<String>(
        value: 'delete',
        padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
        child: Row(
  ...
  void _setAsDefault(FavoriteFilter filter) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('"${filter.name}" set as default')));
  }
  ...
  Widget _buildFilterList(FavoriteFilterList filterList) => ListView.builder(
        ...
        itemBuilder: (context, index) =>
            _buildFilterCard(filterList.filters[index]),
      );
  ...
  Widget _buildFilterCard(FavoriteFilter filter) => Card(
        ...
        child: ListTile(
          ...
          subtitle:
              filter.description != null ? Text(filter.description!) : null,
          trailing: _buildFilterTrailingActions(filter),
        ),
      );

  Widget _buildFilterLeadingIcon(bool isDefault) =>
      Icon(isDefault ? Icons.star : Icons.bookmark_outline,
