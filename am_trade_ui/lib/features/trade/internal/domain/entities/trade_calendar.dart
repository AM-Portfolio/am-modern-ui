...
abstract class TradeCalendar with _$TradeCalendar {
  const factory TradeCalendar(
          {required Map<String, List<TradeDetails>> portfolioTrades}) =
      _TradeCalendar;
  ...
  List<TradeDetails> getTradesForPortfolio(String portfolioId) =>
      portfolioTrades[portfolioId] ?? [];

  /// Get trades by date range
  List<TradeDetails> getTradesByDateRange(
  ...
  List<TradeDetails> getTradesBySymbol(String symbol) => allTrades
      .where((trade) =>
  ...
  double get totalProfitLoss => allTrades.fold(
      0.0, (sum, trade) => sum + (trade.metrics?.profitLoss ?? 0.0));

  /// Get winning trades count
  int get winningTradesCount =>
      allTrades.where((trade) => (trade.metrics?.profitLoss ?? 0.0) > 0).length;
  ...
  int get breakEvenTradesCount => allTrades
      .where((trade) => (trade.metrics?.profitLoss ?? 0.0) == 0)
      .length;
  ...
  Set<String> get uniqueSymbols => allTrades
      .map((trade) => trade.instrumentInfo.symbol ?? '')
...
abstract class TradeCalendarEvent with _$TradeCalendarEvent {
  ...
  factory TradeCalendarEvent.fromTradeDetails(TradeDetails trade) =>
      TradeCalendarEvent(
        id: trade.tradeId,
  ...
  factory TradeCalendarEvent.fromTradeModel(
          TradeModel execution, String portfolioId) =>
      TradeCalendarEvent(
        ...
        amount: (execution.executionInfo?.quantity ?? 0) *
            (execution.executionInfo?.price ?? 0),
