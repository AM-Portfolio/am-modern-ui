...
class UpdateTrade {
  ...
  Future<TradeDetails> call(
    ...
    if (tradeDetails.portfolioId.isEmpty) {
      AppLogger.error('Validation failed - empty portfolioId',
          tag: 'UpdateTrade');
    ...
    try {
      ...
      final result = await _repository.updateTrade(
          tradeId: tradeId, tradeDetails: tradeDetails);

      AppLogger.info('Trade updated successfully - tradeId: ${result.tradeId}',
          tag: 'UpdateTrade');
      AppLogger.methodExit('UpdateTrade.call',
          tag: 'UpdateTrade', result: 'success');

      return result;
    } catch (e) {
      AppLogger.error('Update trade use case failed',
          tag: 'UpdateTrade', error: e, stackTrace: StackTrace.current);
      AppLogger.methodExit('UpdateTrade.call',
          tag: 'UpdateTrade', result: 'error');
      rethrow;
    }
  }
}
