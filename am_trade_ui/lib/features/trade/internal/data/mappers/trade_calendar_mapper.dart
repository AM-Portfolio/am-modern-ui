...
class TradeCalendarMapper {
  ...
  static TradeCalendar fromDto(TradeCalendarDto dto) {
    AppLogger.info(
        '[Mapper] Converting DTO with ${dto.portfolioTrades.length} portfolios',
    ...
    for (final entry in dto.portfolioTrades.entries) {
      ...
      portfolioTrades[portfolioId] =
          tradeDetailDtos.map(_mapTradeDetail).toList();
    }
  ...
  static TradeDetails _mapTradeDetail(TradeDetailsDto dto) => TradeDetails(
        ...
        entryInfo: EntryExitInfo(
          timestamp: dto.entryInfo.timestamp != null
              ? DateTime.tryParse(dto.entryInfo.timestamp!)
              : null,
        ...
        exitInfo: dto.exitInfo != null
            ? EntryExitInfo(
                timestamp: dto.exitInfo!.timestamp != null
                    ? DateTime.tryParse(dto.exitInfo!.timestamp!)
                    : null,
