...
class FavoriteFilterMapper {
  ...
  static FavoriteFilter fromResponseDto(FavoriteFilterResponseDto dto) =>
      FavoriteFilter(
        id: dto.id,
        name: dto.name,
  ...
  static FavoriteFilterRequestDto toRequestDto(FavoriteFilter filter) =>
      FavoriteFilterRequestDto(
        name: filter.name,
...
class MetricsFilterConfigMapper {
  ...
  static MetricsFilterConfig fromDto(MetricsFilterConfigDto dto) =>
      MetricsFilterConfig(
        portfolioIds: dto.portfolioIds ?? [],
        dateRange:
            dto.dateRange != null ? _parseDateRange(dto.dateRange!) : null,
        ...
        tradeCharacteristics: dto.tradeCharacteristics != null
            ? TradeCharacteristicsFilterMapper.fromMap(
                dto.tradeCharacteristics!)
            : null,
        profitLossFilters: dto.profitLossFilters != null
            ? ProfitLossFilterMapper.fromMap(dto.profitLossFilters!)
            : null,
      );

  /// Convert MetricsFilterConfig entity to MetricsFilterConfigDto
  static MetricsFilterConfigDto toDto(MetricsFilterConfig config) =>
      MetricsFilterConfigDto(
        portfolioIds:
            config.portfolioIds.isNotEmpty ? config.portfolioIds : null,
        dateRange: config.dateRange != null
            ? _dateRangeToMap(config.dateRange!)
            : null,
        timePeriod:
            config.timePeriod != null ? {'period': config.timePeriod} : null,
        ...
        tradeCharacteristics: config.tradeCharacteristics != null
            ? TradeCharacteristicsFilterMapper.toMap(
                config.tradeCharacteristics!)
            : null,
...
class InstrumentFilterCriteriaMapper {
      ...
      InstrumentFilterCriteria(
        marketSegments: map['marketSegments'] != null
            ? (map['marketSegments'] as List)
                ...
                .toList()
            : [],
        baseSymbols: map['baseSymbols'] != null
            ? List<String>.from(map['baseSymbols'] as List)
            : [],
        indexTypes: map['indexTypes'] != null
            ? (map['indexTypes'] as List)
                .map((e) => _parseIndexType(e as String))
                .whereType<IndexTypes>()
                .toList()
            : [],
  ...
  static Map<String, dynamic> toMap(InstrumentFilterCriteria criteria) {
    ...
    if (criteria.marketSegments.isNotEmpty) {
      map['marketSegments'] =
          criteria.marketSegments.map(_marketSegmentToString).toList();
    }
    ...
    if (criteria.derivativeTypes.isNotEmpty) {
      map['derivativeTypes'] =
          criteria.derivativeTypes.map(_derivativeTypeToString).toList();
    }
...
class TradeCharacteristicsFilterMapper {
  ...
  static TradeCharacteristicsFilter fromMap(Map<String, dynamic> map) =>
      TradeCharacteristicsFilter(
        strategies: map['strategies'] != null
            ? List<String>.from(map['strategies'] as List)
            : [],
        ...
        statuses: map['statuses'] != null
            ? (map['statuses'] as List)
                .map((e) => _parseTradeStatus(e as String))
                .whereType<TradeStatuses>()
                .toList()
            : [],
  ...
  static Map<String, dynamic> toMap(TradeCharacteristicsFilter filter) {
    ...
    if (filter.directions.isNotEmpty)
      map['directions'] =
          filter.directions.map(_tradeDirectionToString).toList();
    if (filter.statuses.isNotEmpty)
      map['statuses'] = filter.statuses.map(_tradeStatusToString).toList();
    if (filter.minHoldingTimeHours != null)
      map['minHoldingTimeHours'] = filter.minHoldingTimeHours;
    if (filter.maxHoldingTimeHours != null)
      map['maxHoldingTimeHours'] = filter.maxHoldingTimeHours;
...
class ProfitLossFilterMapper {
  ...
  static Map<String, dynamic> toMap(ProfitLossFilter filter) {
    ...
    if (filter.minProfitLoss != null)
      map['minProfitLoss'] = filter.minProfitLoss;
    if (filter.maxProfitLoss != null)
      map['maxProfitLoss'] = filter.maxProfitLoss;
    if (filter.minPositionSize != null)
      map['minPositionSize'] = filter.minPositionSize;
    if (filter.maxPositionSize != null)
      map['maxPositionSize'] = filter.maxPositionSize;
