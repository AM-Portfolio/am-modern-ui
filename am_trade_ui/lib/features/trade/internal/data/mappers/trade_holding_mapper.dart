...
class TradeHoldingMapper {
  ...
  static TradeDetails fromDto(TradeHoldingDto dto) {
    AppLogger.debug('🔄 Mapping TradeHoldingDto to TradeDetails',
        tag: 'TradeHoldingMapper');
    AppLogger.debug('  Trade ID: ${dto.tradeId}', tag: 'TradeHoldingMapper');
    AppLogger.debug(
        '  Attachments in DTO: ${dto.attachments?.length ?? 0} items',
        tag: 'TradeHoldingMapper');

    if (dto.attachments != null && dto.attachments!.isNotEmpty) {
      for (var i = 0; i < dto.attachments!.length; i++) {
        final att = dto.attachments![i];
        AppLogger.debug('    [$i] File: ${att.fileName}, URL: ${att.fileUrl}',
            tag: 'TradeHoldingMapper');
      }
    }
    ...
    AppLogger.debug(
        '  ✅ Mapped Attachments: ${result.attachments?.length ?? 0} items',
        tag: 'TradeHoldingMapper');
    return result;
  }

  /// Convert list of TradeHoldingDtos to list of TradeDetails entities
  static List<TradeDetails> fromDtoList(List<TradeHoldingDto> dtos) =>
  ...
  static TradeHoldings fromListDto(TradeHoldingsDto dto, String portfolioId) {
    final resolvedUserId =
        dto.content.isNotEmpty ? (dto.content.first.userId ?? '') : '';
