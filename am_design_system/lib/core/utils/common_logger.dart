...
class CommonLogger {
  ...
  static void configure(
      {bool enabled = true, LogLevel minLevel = LogLevel.debug}) {
  ...
  static void Function(String action,
      {String? tag, Map<String, dynamic>? metadata})? onUserAction;
  ...
  static void methodExit(String methodName,
      {String? tag, Map<String, dynamic>? metadata}) {
  ...
  static void stateChange(String fromState, String toState,
      {String? tag, Map<String, dynamic>? metadata, String? event}) {
    final metaStr = metadata != null ? ' | DATA: $metadata' : '';
    final eventStr = event != null ? ' | EVENT: $event' : '';
    _log('🔄 STATE: $fromState -> $toState$eventStr$metaStr', LogLevel.debug,
        tag: tag);
  }
  ...
  static void _log(String message, LogLevel level,
    ...
    final finalMessage =
        '$timestamp ${level.emoji} ${level.label}: $tagInfo$message';
