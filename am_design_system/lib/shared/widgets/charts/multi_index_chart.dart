...
class MultiIndexChart extends StatefulWidget {
  /// Preferred input — build from any module's domain data via [MultiSeriesChartData].
  final MultiSeriesChartData? chartData;

  /// Legacy map input (market batch fetch, old callers). Prefer [chartData].
  ...
  /// Convenience for modules still on raw API map shape.
  factory MultiIndexChart.fromLegacyMaps({
    Key? key,
    required Map<String, List<Map<String, dynamic>>> historicalData,
    required List<String> selectedIndices,
    ...
    return MultiIndexChart(
      key: key,
      chartData: MultiSeriesChartData.fromLegacyMaps(historicalData),
...
extension on MultiIndexChart {
  Map<String, List<Map<String, dynamic>>> get _resolvedHistoricalData {
    if (chartData != null) return chartData!.toLegacyMaps();
...
class _MultiIndexChartState extends State<MultiIndexChart> {
  ...
  DateTime? _parseFlexibleDateTime(String str) {
    ...
    // Intraday time-only labels (e.g. 09:15) from portfolio overlay API.
    final timeOnly = RegExp(r'^(\d{1,2}):(\d{2})(?::(\d{2}))?$');
    final match = timeOnly.firstMatch(str.trim());
    if (match != null) {
