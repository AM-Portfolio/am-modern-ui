...
## 3. Manual Basket Creator Specs
**Reference**: `doc/ManualCreator.png`

### **Component: Creation Dashboard**
*   **Layout**: Split view or Wizard flow.
*   **Header**: "Create Custom Basket" with specific "Investment Amount" input.
*   **Search Bar**: Floating glass search bar at the top (`TextField` with `InputDecoration.collapsed`).
*   **Selection Area**:
    *   **Grid**: Large selectable cards for Indices/Themes.
    *   **Drag & Drop**: Ability to drag stocks from "Search" to "My Basket".
*   **Footer**: "Basket Summary" sticky bar showing:
    *   Total Stocks (e.g., 12).
    *   Projected CAGR (Green text).
    *   "Build Basket" CTA Button (Gradient Fill).
