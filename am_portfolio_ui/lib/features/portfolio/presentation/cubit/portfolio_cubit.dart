...
class PortfolioCubit extends Cubit<PortfolioState> {
  ...
  void _performSubscription() {
    ...
    if (!_isSubscribed) {
      CommonLogger.info(
        '[$_debugId] 🎧 PortfolioCubit: Setting up message listener...',
        tag: 'PortfolioCubit',
      );
      ...
      _socketSubscription = _stompClient.messages.listen((frame) {
        CommonLogger.info(
          '[$_debugId] 📨 PortfolioCubit: Frame received from stream!',
          tag: 'PortfolioCubit',
        );

        final destination = frame.headers['destination'];
        CommonLogger.info(
          '[$_debugId] 📍 PortfolioCubit: Destination = $destination',
          tag: 'PortfolioCubit',
        );

        if (destination != null && destination.contains('portfolio')) {
          CommonLogger.info(
            '✅ PortfolioCubit: Destination matches "portfolio"',
            tag: 'PortfolioCubit',
          );

          if (frame.body != null) {
            CommonLogger.info(
              '📦 PortfolioCubit: Frame has body, length = ${frame.body!.length}',
              tag: 'PortfolioCubit',
            );

            try {
              CommonLogger.info(
                '⚙️ PortfolioCubit: Parsing JSON...',
                tag: 'PortfolioCubit',
              );
              final json = jsonDecode(frame.body!);

              CommonLogger.info(
                '💾 PortfolioCubit: Calling updateSummaryFromSocket with currentValue=${json['currentValue']}',
                tag: 'PortfolioCubit',
              );
              updateSummaryFromSocket(json);
            } catch (e) {
              CommonLogger.error(
                '❌ PortfolioCubit: Error parsing WebSocket message',
                error: e,
                tag: 'PortfolioCubit',
              );
            }
            ...
            CommonLogger.warning(
              '⚠️ PortfolioCubit: Frame body is null',
              tag: 'PortfolioCubit',
            );
          }
        } else {
          CommonLogger.debug(
            '⏭️ PortfolioCubit: Skipping frame, destination does not contain "portfolio"',
            tag: 'PortfolioCubit',
          );
        }
      });
      ...
      _isSubscribed = true;
      CommonLogger.info(
        '✅ PortfolioCubit: Message listener setup complete',
        tag: 'PortfolioCubit',
      );
    }
  }
  ...
  Future<void> loadPortfolio(String userId) async {
    ...
    try {
      ...
      if (!isClosed) {
        emit(
          PortfolioError(error.toString(), portfolioList: state.portfolioList),
        );
      }
  ...
  Future<void> loadPortfoliosList(String userId) async {
    ...
    emit(PortfolioListLoading(portfolioList: state.portfolioList));
  ...
  void updateSummaryFromSocket(Map<String, dynamic> json) {
    ...
    if (currentState is PortfolioLoaded) {
      try {
        CommonLogger.debug(
          'Processing WebSocket update in Cubit for state: PortfolioLoaded',
          tag: 'PortfolioCubit',
        );
        // 1. Parse DTO using the specific Socket DTO
        final dto = PortfolioSocketUpdateDto.fromJson(json);

        // 1.1 Validate portfolio matching if portfolioId is present in update
        if (dto.portfolioId != null &&
            dto.portfolioId != currentState.portfolioId) {
          CommonLogger.warning(
            '⏭️ Skipping WebSocket update: Portfolio mismatch. Received: ${dto.portfolioId}, Current: ${currentState.portfolioId}',
            tag: 'PortfolioCubit',
          );
          return;
        }
        ...
        CommonLogger.info(
          '📨 Received socket update for portfolio: ${dto.portfolioId ?? 'current'}',
          tag: 'PortfolioCubit',
        );
        ...
        if (dto.equities.isNotEmpty) {
          ...
          updatedHoldings = currentState.holdings
              .map((existing) {
                ...
                if (update.isin.isNotEmpty || update.symbol.isNotEmpty) {
                  return existing.copyWith(
                    name: update.name.isNotEmpty ? update.name : existing.name,
        ...
        CommonLogger.info(
          'State updated from WebSocket: Val=${dto.currentValue}',
          tag: 'PortfolioCubit',
        );
      } catch (e) {
        CommonLogger.error(
          'Failed to update summary from socket',
          error: e,
          tag: 'PortfolioCubit',
        );
      }
