...
class SectorHeatmapConverter {
  ...
  static bool _shouldUseAllocationFallback(
    ...
    // PROD: live heatmap often returns many sectors with totalValue=0 while
    // allocation has the full book (~₹9L). 5% was too low (e.g. 5.7% kept
    // a 2-tile treemap). Require ≥50% mass vs allocation.
    if (valueCoverage < 0.5) {
      return true;
    }
    ...
    // Zero-value sectors must not count as "complete" structure.
    final namedHeatmapCount = resolved
        .where(
          (sector) =>
              !isInvalidSectorName(sector.sectorName) && sector.totalValue > 0,
        )
        .length;
  ...
  static String _formatChangePercent(double value) =>
      StringUtils.formatSignedPercent(value);
