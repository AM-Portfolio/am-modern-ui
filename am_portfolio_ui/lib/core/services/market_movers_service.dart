...
class MarketMoversService {
  ...
  Future<MarketMoversData> fetchMarketMovers({
    ...
    try {
      final baseUrl =
          common.ConfigService.config.api.marketData?.baseUrl ??
      ...
      CommonLogger.info(
        'Fetching global market movers from $uri',
      ...
      final response = await http
          ...
          .timeout(
            ...
            onTimeout: () =>
                throw Exception('Connection timeout fetching market movers'),
          );

      if (response.statusCode == 200) {
        CommonLogger.info(
          'Market movers response: ${response.body}',
          tag: 'MarketMoversService',
        );
        ...
        return MarketMoversData(
          gainers: gainersList
              .map((e) => MarketMoverStock.fromJson(e))
              .toList(),
        ...
        throw Exception(
          'Failed to fetch market movers: ${response.statusCode}',
        );
      }
    } catch (e) {
      CommonLogger.error(
        'Error in fetchMarketMovers',
        tag: 'MarketMoversService',
        error: e,
      );
